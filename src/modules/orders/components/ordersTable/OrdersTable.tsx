...
import OrdersRowTable from "./OrdersRowTable";
...
function OrdersTable() {
  ...
  const TABLE_HEADERS: {
    ...
    className?: string;
      ...
      {
        Icon: <Banknote className="h-4 w-4" />,
        label: tHeaders("productsCouponValue"),
      },
      {
        Icon: <Banknote className="h-4 w-4" />,
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        <ReusableTable
          ...
          className=""
          renderRow={(order) => (
            <OrdersRowTable key={order.id} data={{ ...order, onUpdated: refetch }} />
          )}
        />
      </div>
    </div>
