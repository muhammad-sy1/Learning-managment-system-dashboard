...
import { IGetSectionResponse } from "../types/section";

export async function fetchSectionsClient(filters: SectionFiltersSchema) {
  const { parent_id, ...res } = filters;
  try {
    const response = await fetcherClient.get<IGetSectionResponse>(
      endpoints.getSections, // Update endpoint
      {
        params: {
          // ...filters,
          ...res,
