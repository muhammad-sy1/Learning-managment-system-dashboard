...
export const addCouponSchema = (t: (key: string) => string) =>
  z
    .object({
      code: z
        .string()
      ...
      product_ids: z.array(z.string()).optional().default([]).optional(),
      merchant_ids: z.array(z.string()).optional().default([]),
      applies_to: z
    ...
    .superRefine((data, ctx) => {
      ...
      if (
        ...
        data.applies_to !== "CUSTOM_ORDER_SHIPPING"
      ) {
        ctx.addIssue({
          path: ["product_ids"],
          code: z.ZodIssueCode.custom,
...
export type AddCouponSchema = z.input<ReturnType<typeof addCouponSchema>>;
